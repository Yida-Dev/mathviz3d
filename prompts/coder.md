...
| ID | Description |
|----|-------------|
| geometry | Overall geometry (PREFERRED) |
| cube | Cube (when baseGeometry.type = "cube") |
| tetrahedron | Tetrahedron (when baseGeometry.type = "tetrahedron") |
| square | Square (when baseGeometry.type = "square") |
| vertexLabels | All vertex labels |
| center | Geometric center point |

**INVALID Targets** (common mistakes):
| Wrong | Correct | Reason |
|-------|---------|--------|
| baseGeometry | geometry | "baseGeometry" is a schema field, not a target |
| cube (when type != cube) | geometry | Use "geometry" when uncertain |
| edges | geometry | "edges" is not a valid target |
| vertices | vertexLabels | Use "vertexLabels" to show all labels |
